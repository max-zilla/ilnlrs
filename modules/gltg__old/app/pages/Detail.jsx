...
class Detail extends Component {
    ...
    setUpCategoryMappings(newProps) {
        const sensor = newProps.sensors.find(x => x.name === newProps.match.params.name);
        if (sensor) {
    ...
    render() {
        ...
        if (Object.keys(this.state.category_mappings).length === 0) {
            ...
            if (this.props.parameters.failed) {
            ...
            else if (this.props.selected_detail !== this.props.match.params.name) {
                contents = (
                    <span>
                ...
                contents = (<span><h1>{this.props.match.params.name}</h1><Spinner/></span>);
            }
            return (
                <div>
        ...
        let selected_category = Object.keys(category_mappings).indexOf(this.props.match.params.category) > 0 ? this.props.match.params.category :
            Object.keys(category_mappings)[0];
        // TODO: Show a loading screen while getting the sensor if categories_mapping is empty
        if (screen.width > getMobileSizeMax()) {
            display_tabs = <DetailTabs sensorName={this.props.match.params.name} categories={this.state.category_mappings}
